...
const useCoinDetailsPreloader = (
  ...
  const initiateFetchIfNotPreloading = useCallback(() => {
    ...
    console.warn("isBeingPreloadedGlobally", isBeingPreloadedGlobally);
    console.warn(
      "isPreloadingDetailsForCurrentCurrency",
      isPreloadingDetailsForCurrentCurrency,
    );
    console.warn("existingPreloadedDetails", existingPreloadedDetails);
    ...
    if (
      ...
      console.warn(
        `Attempted to preload coin '${symbol}' which is already ${
          existingPreloadedDetails != null ? "" : "being "
        }preloaded.`,
      );
      return;
    }
