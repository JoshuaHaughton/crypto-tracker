...
const Coin = ({
  ...
  useEffect(() => {
    const updateCurrencyValues = () => {
      ...
      const updatedCoinPrice =
        coin.current_price * rates[currentCurrency.toUpperCase()];
      const updatedMarketCap =
        coin.market_cap * rates[currentCurrency.toUpperCase()];
      const updatedAth =
        coin.all_time_high * rates[currentCurrency.toUpperCase()];
      const updatedPriceChange1d =
        coin.price_change_1d * rates[currentCurrency.toUpperCase()];
      ...
      setMarketChart((prevState) => ({
        day: prevState.day.map((dataPoint) => [
          dataPoint[0],
          dataPoint[1] * rates[currentCurrency.toUpperCase()],
        ]),
    ...
    const prefetchHomePage = async () => {
      ...
      const allFormattedCoins = marketCapCoinsData.Data.map((entry, i) => {
        ...
        return {
          ...
          current_price: metrics.PRICE * rates[currentCurrency.toUpperCase()],
          current_price_USD: metrics.PRICE,
          current_price_AUD: metrics.PRICE * rates.AUD,
          current_price_GBP: metrics.PRICE * rates.GBP,
          market_cap: metrics.MKTCAP * rates[currentCurrency.toUpperCase()],
          market_cap_rank: i + 1,
          total_volume:
            metrics.TOTALVOLUME24HTO * rates[currentCurrency.toUpperCase()],
          high_24h: metrics.HIGH24HOUR * rates[currentCurrency.toUpperCase()],
          low_24h: metrics.LOW24HOUR * rates[currentCurrency.toUpperCase()],
          price_change_24h:
            metrics.CHANGE24HOUR * rates[currentCurrency.toUpperCase()],
    ...
    if (firstRender) {
      setFirstRender(false); // Set firstRender to false for subsequent renders
    } else {
...
export async function getServerSideProps(context) {
  ...
  const urls = [
    // coin info
    `https://min-api.cryptocompare.com/data/pricemultifull?fsyms=${id.toUpperCase()},USD&tsyms=CAD,USD,AUD,GBP`,
    `https://data-api.cryptocompare.com/asset/v1/data/by/symbol?asset_symbol=${id.toUpperCase()}`,
    // chart data
    `https://min-api.cryptocompare.com/data/v2/histohour?fsym=${id}&tsym=${currency}&limit=24`, // 1 day
    `https://min-api.cryptocompare.com/data/v2/histoday?fsym=${id}&tsym=${currency}&limit=365`, // 1 year
    // get paprika id to search for ath
  ...
  // Extract rates and coin details
  const rates = {
    USD: 1,
    CAD: cryptoCompareData.RAW.USD.CAD.PRICE,
    AUD: cryptoCompareData.RAW.USD.AUD.PRICE,
    GBP: cryptoCompareData.RAW.USD.GBP.PRICE,
  };
  ...
  const weekData = {
    Data: {
      Data: yearData.Data.Data.slice(-7), // Last 7 days data
    },
  };
  const monthData = {
    Data: {
